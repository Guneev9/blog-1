...
Residual Plots (yes, again) can be used to identify outliers. We can look at the the last residual plot above. Usually, the observations with absolute standard residuals above 3 are possible outliers.
...
#print the number of identified outliers as per absolute standard residuals
length(outlier.ids)
```
...
We can't compare the $R^2$ value of the models directly now since the model is built by removing the outliers.
...
fitted_vs_actual <- function(predictions){
...
plot_effect <- function(interaction, xpredictor){
  ...
  effs <- effect(interaction, mod = lm.fit2, 
