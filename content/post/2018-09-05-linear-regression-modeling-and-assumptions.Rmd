...
We can't compare the R-squared of the models directly now since the model is built by removing the outliers.
...
predictions_init <- predict(lm.fit, insurance, interval = "confidence") %>%
  tidy()

fitted_vs_actual(predictions_init)
```
...
```{r}
predictions_final <- predict(lm.fit2, insurance, 
                             interval = "confidence")^(1/yTransformer) %>%
  tidy()

fitted_vs_actual(predictions_final)
```

This is quite an improvement from the initial model. The fitted values are closer to the actual charges.
